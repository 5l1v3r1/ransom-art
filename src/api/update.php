    ...
    if ($raw) {
        ...
        if ($data) {
            ...
            if (isset($data->user) && isset($data->machine) && isset($data->email) && isset($data->bitcoin)) {
                ...
                $error = false;
                if (mb_strlen($parameters['user']) < 1 || mb_strlen($parameters['user']) > 30) {
                    $error = true;
                }
                if (mb_strlen($parameters['machine']) < 1 || mb_strlen($parameters['machine']) > 30) {
                    $error = true;
                }
                if (mb_strlen($parameters['email']) < 1 || mb_strlen($parameters['email']) > 300) {
                    $error = true;
                }
                if (mb_strlen($parameters['bitcoin']) < 1 || mb_strlen($parameters['bitcoin']) > 60) {
                    $error = true;
                }
                if (!$error) {
                    $db = new Database();
